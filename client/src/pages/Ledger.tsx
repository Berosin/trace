...
export function Ledger() {
  ...
  return (
    <div className="px-10 py-8 max-w-4xl">
      <SectionLabel index="003">Provenance</SectionLabel>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-mono text-2xl font-bold text-ink tracking-tight">Global ledger</h1>
        {integrity && (
          <span
            className={`inline-flex items-center gap-1.5 text-[11px] font-mono uppercase tracking-wide font-bold px-3 py-1.5 border ${
              integrity.valid ? "bg-success/10 text-success border-success" : "bg-danger/10 text-danger border-danger"
            }`}
          >
      ...
      {ledger.length === 0 ? (
        ...
        <div className="border-2 border-ink bg-panel shadow-panel">
          <ol className="divide-y divide-ink/15 max-h-[70vh] overflow-y-auto scrollbar-thin">
            {[...ledger].reverse().map((entry) => (
              <li key={entry.id} className="px-5 py-3.5">
                <div className="flex items-center gap-2 text-[11px] font-mono text-muted mb-1">
                  ...
                  <span>·</span>
                  <span className="text-accent">{shortIdFor(entry.ticketId)}</span>
                  <span>·</span>
                  <span>{ACTOR_LABEL[entry.actor] ?? entry.actor}</span>
                </div>
                <p className="text-sm text-ink/85 leading-snug">{entry.summary}</p>
                <p className="text-[10px] font-mono text-ink/30 mt-1 truncate">hash {entry.hash.slice(0, 24)}…</p>
              </li>
            ))}
          </ol>
        </div>
